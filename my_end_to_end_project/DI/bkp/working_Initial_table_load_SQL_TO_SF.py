...
for tbl in TABLES:
    ...
    """)
    sschema = pd.read_sql_query(sql, sqlserver_engine, params={"tbl":tbl})
    if sschema.empty:
        logger.warning(f"  • No columns found in SQL Server for `{tbl}`; skipping.")
        continue

    # 5b) Introspect existing SF columns
    with sf_conn.cursor() as cur:
        cur.execute(
            """
            SELECT COLUMN_NAME
    ...
    # 5d) Extract full data from SQL Server
    ...
    # 5e) Bulk-load via write_pandas on sf_conn
...
logger.info("✅ All tables synced successfully, no quoted identifiers.")